...
Kame Finance runs due automations and payment reminders via **`GET /api/cron/dispatch`**. Production uses **Supabase `pg_cron` + `pg_net`** **once daily** (`0 4 * * *` UTC = 12:00 PM Asia/Manila), matching **`apps/web/vercel.json`**. Overdue jobs still run on the next tick when `next_run_at` is in the past.
...
```sql
select vault.create_secret('https://<your-vercel-host>', 'kame_finance_app_url');
select vault.create_secret('<CRON_SECRET from Vercel>', 'kame_finance_cron_secret');
```

Use the current production host (for example `kame-ops-web.vercel.app` until the Vercel project is renamed). To rotate: delete old vault rows and create new ones, then re-run sync (step 3).

If Vault still has `kame_ops_app_url` / `kame_ops_cron_secret`, create the new secret names above, then run the updated snippet so it unschedules `kame-ops-automation-dispatch` and installs `kame-finance-automation-dispatch`.
...
```sql
select public.sync_kame_finance_automation_dispatch_cron_job();
```

Expected: `{"ok": true, "job": "kame-finance-automation-dispatch", "cronExpr": "0 4 * * *"}`

### 4. Verify

```sql
select jobid, jobname, schedule, active from cron.job
where jobname = 'kame-finance-automation-dispatch';
```
...
select cron.unschedule('kame-finance-automation-dispatch');
...
```bash
curl -sS -H "Authorization: Bearer $CRON_SECRET" \
  "https://kame-finance.vercel.app/api/cron/dispatch"
```
...
| `401 Unauthorized` from dispatch    | `kame_finance_cron_secret` in Vault must match Vercel `CRON_SECRET`                                   |
| Jobs never run at scheduled time    | Daily cron fires at 04:00 UTC; user schedule is timezone-aware (`users.timezone`)                 |
| `sync_…` returns missing Vault      | Create both `kame_finance_app_url` and `kame_finance_cron_secret` first                                   |
...
- Redeployed the correct `'0 4 * * *'` cron function and re-ran `sync_kame_finance_automation_dispatch_cron_job()` (verified `cron.job.schedule = '0 4 * * *'`).
...
```sql
select jobid, schedule, active from cron.job where jobname = 'kame-finance-automation-dispatch';
```
